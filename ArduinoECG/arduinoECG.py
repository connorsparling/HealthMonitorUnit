...
@sio.on('test-segment')
def test_ecg(data):
    print_log('test-segment')
    result = evaluateNNData(data)
    if result >= 0:
        print_log("FUCK YOU'RE GONNA DIE => " + BEAT_TYPES[ARRYTHMIA_TYPES_INDEX[result]])
        sio.emit('alert', BEAT_TYPES[ARRYTHMIA_TYPES_INDEX[result]])
    else:
...
@sio.event
def connect():
    print_log("I'm connected!")

@sio.event
def connect_error():
    print_log("The connection failed!")

@sio.event
def disconnect():
    print_log("I'm disconnected!")
...
def socketIOEmitQueue(threadname):
    ...
    while runThreads:
        try:
            item = emit_queue.get()
            print_log("GOT EMIT ITEM")
            if item is not None:
                print_log(item['function'])
